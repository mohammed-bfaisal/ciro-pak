...
import { removeMissionBriefingLayers, syncMissionBriefingLayers } from './missionBriefingLayers';
...
export function CiroMap({ city, onCrisisClick }: CiroMapProps) {
  ...
  useEffect(() => {
    if (!mapInstance.current) return;
    const map = mapInstance.current;
    const sync = () => syncMissionBriefingLayers(map, city, missionBriefingEnabled);
    if (map.isStyleLoaded()) sync();
    else map.once('load', sync);
  ...
  useEffect(() => {
    return () => {
      if (mapInstance.current) {
        ...
        removeMissionBriefingLayers(mapInstance.current);
