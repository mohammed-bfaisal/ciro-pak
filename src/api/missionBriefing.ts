...
export async function checkMissionBriefingStatus(options: P04ApiOptions = {}): Promise<P04BackendStatus> {
...
export async function simulateMissionBriefing(
  options: P04SimulationOptions,
): Promise<P04SimulationResponse> {
  const nowIso = (options.now ?? (() => new Date()))().toISOString();
  const baseUrl = options.baseUrl === undefined ? getBackendBaseUrl() : options.baseUrl;
  if (!baseUrl) return createFallbackP04Simulation(nowIso, options.request.action);
  ...
  try {
    ...
    if (!response.ok) return createFallbackP04Simulation(nowIso, options.request.action);
    return await response.json() as P04SimulationResponse;
  } catch {
    return createFallbackP04Simulation(nowIso, options.request.action);
  }
}
